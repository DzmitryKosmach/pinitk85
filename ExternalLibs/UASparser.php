...
class UASparser
{
     ...
     * @type integer
     */
    public $updateInterval = 172800; // 2 day
    ...
    public function parse($useragent = null)
    {
        ...
        $result["device_icon"] = "unknown.png";
        $result["device_info_url"] = "unknown";
        ...
        if (!$os_id) { // Search for the OS by reg
        ...
        // Search device
        foreach ($this->data['device_reg'] as $test) {
            if (@preg_match($test[0], $useragent)) {
                $device_id = $test[1];
                break;
            }
        ...
        // A valid device was found
        if ($device_id) { // device detail
            $device_data = $this->data['device'][$device_id];
            if ($device_data[0]) {
                $result['device_type'] = $device_data[0];
            }
            if ($device_data[0]) {
                $result['device_icon'] = $device_data[0];
            }
            if ($device_data[0]) {
                $result['device_info_url'] = $device_data[0];
            }
        }
        else {// based on type
            if( $result['typ'] == "Other" || $result['typ'] == "Library" || $result['typ'] == "Validator" || $result['typ'] == "Useragent Anonymizer" ){
                $result["device_type"] = $this->data['device'][1][0];
                $result["device_icon"] = $this->data['device'][1][1];
                $result["device_info_url"] = self::$info_url . $this->data['device'][1][2];
            }
            else if($result['typ'] == "Mobile Browser" || $result['typ'] == "Wap Browser") {
                $result["device_type"] = $this->data['device'][3][0];
                $result["device_icon"] = $this->data['device'][3][1];
                $result["device_info_url"] = self::$info_url . $this->data['device'][3][2];
            }
            else{
                $result["device_type"] = $this->data['device'][2][0];
                $result["device_icon"] = $this->data['device'][2][1];
                $result["device_info_url"] = self::$info_url . $this->data['device'][2][2];
            }
        }
