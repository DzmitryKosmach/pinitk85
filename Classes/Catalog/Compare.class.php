...
class Catalog_Compare {
	...
	/**
	 * Максимум серий в списке сравнения
	 */
	const MAX = 4;
	...
	static function add($seriesId){
		...
		//print implode('-', array_keys($_SESSION[self::SESS_KEY])) . '-';
		if(count($_SESSION[self::SESS_KEY]) > self::MAX){
			$sIds = array_keys($_SESSION[self::SESS_KEY]);
			$sIds = array_slice($sIds, count($sIds) - self::MAX);

			$_SESSION[self::SESS_KEY] = array();
			foreach($sIds as $sId) $_SESSION[self::SESS_KEY][$sId] = 1;
		}
	}
	...
	/** Являются ли все значения входного массива эквивалентными (чтобы показать это в сравнении серий)
	 * @static
	 * @param	array	$values		Массив строк. чисел или массивов строк/чисел
	 * @return	bool
	 */
	static function isEqual($values){
