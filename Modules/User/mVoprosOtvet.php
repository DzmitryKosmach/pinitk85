...
class ModuleMain
{
  public static function main($pageInf)
  {
    ...
    $tpl = Config::path('skins') . '/html/User/mVoprosOtvet.htm';
      ...
      if (count($formErrors) === 0) {
        try {
          $ok = Contacts_Feedback::send(
            ...
            $phone,
            $text,
            Contacts_Feedback::REASON_OTHER
          );
    ...
    $data = array(
      'pageInf' => $pageInf,
      'title' => $pageInf['name'] ?? 'Вопрос-Ответ'
    );

    return pattExeP(fgc($tpl), $data);
  }

  /** На проде mebelioni.ru — абсолютный URL; иначе редирект на тот же хост (удобно для локальной проверки). */
  ...
  /**
   * Та же логика, что в Form::validate() (reCAPTCHA v2).
   */
