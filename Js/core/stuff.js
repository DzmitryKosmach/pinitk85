...
$(document).ready(function(){
  ...
  $(document).on('click','.add2compare',function(e){
    ...
    if(id>0) {
      ...
      ks = window.location.search;
      var url = ks+'/compare/?'+action+'=' + id + '&ajax=1';
      $.getJSON(url, {}, function(json){

        RSGoPro_PutJSon(json);
        if( action=='remove' ) // delete from compare
        {
  ...
  RSGoPro_FancyOptions1 = {
    ...
    padding			: 20,
    tpl				: {
      closeBtn : '<a title="Close" class="fancybox-item fancybox-close absolute top-2 right-2 w-12 h-12 flex items-center justify-center text-5xl text-black-500 z-10 rounded-full hover:bg-red-400 transition-colors" href="javascript:;">×</a>',
    },
    helpers			: {
      title : {
        type : 'inside',
