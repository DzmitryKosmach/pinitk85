...
function bookmark(a) {
    ...
    if (window.opera) {
        ...
        // IE
        var version = Math.round(
            navigator.appVersion.charAt(
                navigator.appVersion.indexOf("MSIE") + 5
            )
        );
        if (version >= 4 && window.external)
            window.external.addFavorite(url, title);
    } else if (navigator.appName && window.sidebar) {
        // Другие браузеры
...
var AJAX = new (function () {
    ...
    this.submitForm = function (event, form, callback) {
        ...
        this.lookup(url, callback, postData);
...
var oMultiselectCount = new (function (selectId, cntElementId) {
    ...
    this.add = function (selectId, cntElementId) {
        addHandler($$$(selectId), "change", function () {
...
function translit(text) {
    ...
    if (text)
        for (i = 0; i < eng.length; i++)
            while (text.indexOf(rus[i]) != -1)
                text = text.replace(rus[i], eng[i]);
    return text;
}
    ...
    for (var i in o)
        if (typeof o[i] == "object")
            res +=
                ...
                "]" +
                " => " +
                print_r(o[i], sep, tabs2) +
                sep;
        else res += tabs2 + "[" + i + "]" + " => " + o[i] + sep;
...
function makeDraggable(el, onDragStart, onDrag, onDragStop) {
    ...
    addHandler(el, "mousedown", function (e) {
        e = e || window.event;
    ...
    addHandler(D, "mousemove", function (e) {
        if (!drag) return;
        e = e || window.event;
        eventCancelDefault(e);
...
onLoad(function () {
    ...
    for (var i = 0, l = series.length; i < l; i++) {
        ...
        addHandler(series[i], "mouseover", function () {
...
function showItemPopup1(url, itemId) {
    ...
    oPopup.loadUrl("", url, false, function () {
        oMaterials.refresh();
        oMaterials.onChangePageSize = function (subOpened) {
            if (subOpened) {
                findOne(".fancybox-inner").style.height =
                    winSize().h - 130 + "px";
            } else {
        ...
        oMaterials.onSave = function (price, material) {
            ...
            var matsPreviewsBlock = $$$(
                "item1-" + itemId + "-materials-preview"
            );
            ...
            if (matsPreviewsBlock) {
                ...
                matsPreviewsBlock.className =
                    "materials pos-" + Math.ceil(previewNum / 7);
            }
        };
    ...
    oPopup.loadUrl("", url, false, function () {
        ...
        oMaterials.onSave = function (price, material) {
            ...
            if (price.current) {
                ...
                $$$("item2-" + itemId + "-price").innerHTML = priceFormat(
                    price.current
                );
                if (price.old) {
                    $$$("item2-" + itemId + "-price-old").style.display = "";
                    $$$("item2-" + itemId + "-price-old").innerHTML =
                        priceFormat(price.old);
                } else {
                    $$$("item2-" + itemId + "-price-old").style.display =
                        "none";
                ...
                if ($$$("item2-" + itemId + "-price-in")) {
                    $$$("item2-" + itemId + "-price-old").style.display = "";
                    $$$("item2-" + itemId + "-price-in").innerHTML =
                        priceFormat(price.in);
                }
            } else {
        ...
        oMaterials.open(itemId, $$$("item2-" + itemId + "-material").value);
...
var oMaterials = new (function () {
    ...
    this.sortSubByOrder2 = function (obj) {
        ...
        const subEntries = Object.entries(newObj.sub).sort(
            (a, b) => parseInt(a[1].order, 10) - parseInt(b[1].order, 10)
        );
    ...
    this.scrollToSectionPhoto = function () {
        ...
        setTimeout(function () {
            ...
            const container = block.closest(".fancybox-inner") || document.querySelector(".fancybox-inner");
        ...
        if (price > 0) {
            ...
            if (priceOldEl) {
                ...
                priceOldEl.innerHTML =
                    priceFormat(matPriceOld4Item(topM.id)) + " ₽";
            }
            ...
            if (priceInEl) {
                priceInEl.style.display = "";
                priceInEl.innerHTML =
                    priceFormat(matPriceIn4Item(topM.id)) + " ₽";
            }
        } else {
...
var oCompare = new (function () {
    ...
    this.add = function (seriesId) {
        ...
        var addButtons = document.querySelectorAll(
            '.series-compare-add[data-id="' + seriesId + '"]'
        );
        var removeButtons = document.querySelectorAll(
            '.series-compare-remove[data-id="' + seriesId + '"]'
        );
...
function Search() {
    ...
    this.init = function () {
      ...
      return;
        ...
        addHandler(this.inp, "blur", function () {
            oSearch.hide();
        });
    ...
    this.request = function (text) {
        ...
        // AJAX-запрос
        //serachAjaxDemo(
        AJAX.lookup(this.url + encodeURIComponent(text), function (res) {
    ...
    this.show = function (res, text) {
        ...
        for (var i in res) {
            if (trim(res[i]) !== "") {
                ...
                variant = variant.replace(
                    new RegExp(RegExp.escape(text), "ig"),
                    "<b>$&</b>"
                );
                html +=
                    '<div onmouseover="oSearch.variantCheck(this)" onclick="oSearch.select(this)">' +
                    variant +
                    "</div>";
                resCount++;
            }
        ...
        if (TRANSITION_SUPPORT) {
            setTimeout(function () {
                oSearch.layer.style.height =
                    oSearch.resHeight * resCount + 10 + "px";
            }, 10);
        } else {
    ...
    this.show2 = function (html) {
        ...
        if (TRANSITION_SUPPORT) {
            setTimeout(function () {
                oSearch.layer.style.height =
                    oSearch.resHeight * resCount + 119 + "px";
            }, 10);
        } else {
...
var oCart = new (function () {
    ...
    jQuery(document).on(
        "click",
        '.seriespage-left div.add2basketform.main-cart-form button[type="submit"]',
        function () {
...
function goToTop() {
    ...
    function checkScroll() {
        if (!link) return;
        var scrollPos = D.body.scrollTop
            ? D.body.scrollTop
            : D.documentElement.scrollTop;
        if (scrollPos >= 500) {
...
document.addEventListener("DOMContentLoaded", function () {
    if (document.querySelector(".tags_container")) {
        ...
        var h =
            $(".tags_container").offset().top -
            tPanelHeight -
            $(window).scrollTop();
        if (h < 0) {
            $("#tags").addClass("scrolled");
            document.querySelector(".tags_container").style.marginBottom =
                elHeight + "px";
    ...
    if ($(window).width() < 876) {
        if (document.querySelector("#tags")) {
            var tagPages = $(".tag_pages"),
                ...
                tagPagesLinkHeight =
                    tagPagesLink.outerHeight() +
                    parseInt(tagPagesLink.css("margin-top"), 10) +
                    parseInt(tagPagesLink.css("margin-bottom"), 10),
            ...
            if (tagPagesHeight > tagPagesLinkHeight) {
                ...
                tagPages.css(
                    "height",
                    tagPagesLinkHeight - tagPagesLinkBorderHeight + "px"
                );
            }
        ...
        var aroundText = document.querySelector(
            ".new_block .searchinhead .aroundtext"
        );
        var icon1 = document.querySelector(".new_block .searchinhead .icon1");
        var catalogMenuParent = document.querySelector(
            ".new_block .catalogmenu li.parent"
        );
        if (aroundText && icon1) {
        ...
        if (aroundText) {
            $(window).scroll(function () {
                var searchInput = document.querySelector(
                    ".new_block .searchinhead .aroundtext input"
                );
                if (
                    searchInput &&
                    !searchInput.value &&
                    !$(".new_block .searchinhead .aroundtext input").is(
                        ":focus"
                    )
                ) {
        ...
        var catalog_button = document.querySelector(
                ".new_block .catalogmenu .icon.menu"
            ),
            clearfix = document.querySelector(
                ".new_block .catalogmenu.clearfix"
            ),
            liParent = document.querySelector(
                ".new_block .catalogmenu li.parent"
            );
