<?php
/***************************************************************************\
| Sypex Geo                  basic version 2.2.3                            |
| (c)2006-2014 zapimir       zapimir@zapimir.net       http://sypex.net/    |
| (c)2006-2014 BINOVATOR     info@sypex.net                                 |
|---------------------------------------------------------------------------|
| created: 2006.10.17 18:33      modified by Mik Foxi: 2025.06.03 00:11     |
| Имя функции SxGeo изменено на SxGeoAb (чтобы избежать конфликты имен)     |
| добавлена совместимость с современными версиями PHP до 8.4                |
|---------------------------------------------------------------------------|
...
class SxGeoAb {
    ...
    protected $b_idx_len;    // Добавлено для line 63
    protected $id_len;       // Добавлено для line 66
    protected $block_len;    // Добавлено для line 67
    protected $max_region;   // Добавлено для line 68
    protected $max_city;     // Добавлено для line 69
    protected $max_country;  // Добавлено для line 70
    protected $pack;         // Добавлено для line 74
	...
	public function get_num($ip){
		...
		if ($this->memory_mode) {
			...
			if ($len <= 0) return null; // 2025.06.03
